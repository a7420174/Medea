...
class LiteratureSearch(BaseAction):
    ...
    def __init__(self, model_name=os.getenv("BACKBONE_LLM"), verbose=True) -> None:
        ...
        self.semantic_scholar = SemanticScholarSearch(
            extractor=self.keyword_extractor, verbose=verbose
        )
    ...
    def __call__(
        ...
        max_papers = (
            int(max_papers) if isinstance(max_papers, str) else (max_papers or 16)
        )
        min_citation_count = (
            int(min_citation_count)
            if isinstance(min_citation_count, str)
            else (min_citation_count or 0)
        )
        ...
        try:
            ...
            if include_openalex:
                ...
                if openalex_papers and isinstance(openalex_papers, list):
                    ...
                    if self.verbose:
                        print(
                            f"[LiteratureSearch] Found {len(openalex_papers)} papers from OpenAlex"
                        )
            ...
            if ss_papers:
                ...
                if self.verbose:
                    print(
                        f"[LiteratureSearch] Found {len(ss_papers)} papers from Semantic Scholar"
                    )
            ...
            if self.verbose:
                print(
                    f"[LiteratureSearch] Final results: {len(final_papers)} unique papers from {', '.join(collection.sources_used)}"
                )
...
class PaperJudge(BaseAction):
    ...
    def _evaluate_single_paper(
        self, paper_idx: int, paper: Dict, user_query: str, total_papers: int
    ):
        ...
        try:
            ...
            if self.verbose:
                print(
                    f"[PaperJudge] Evaluating paper {paper_idx + 1}/{total_papers}",
                    flush=True,
                )
    ...
    def __call__(
        self,
        user_query: str,
        literature_collection: LiteratureCollection,
        ...
        if self.verbose:
            print(
                f"[PaperJudge] Evaluating {len(papers)} papers in {literature_collection.get_id()} for relevance to: {user_query}"
            )
            print(
                f"[PaperJudge] Using parallel processing with max_workers={self.max_workers}"
            )
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                ...
                future_to_paper = {
                    executor.submit(
                        self._evaluate_single_paper, i, paper, user_query, len(papers)
                    ): i
                ...
                for future in as_completed(future_to_paper):
                    ...
                    if error:
                        ...
                        if self.verbose:
                            print(
                                f"[PaperJudge] Error evaluating paper {error[0] + 1}: {error[1]}",
                                flush=True,
                            )
                    else:
            ...
            if self.verbose:
                print(
                    f"[PaperJudge] Assessment complete: {len(relevant_papers)}/{len(papers)} papers deemed relevant"
                )
                if errors:
                    print(
                        f"[PaperJudge] {len(errors)} errors occurred during evaluation"
                    )
...
class OpenScholarReasoning(BaseAction):
    ...
    def __init__(
        self,
        llm_provider=os.getenv("BACKBONE_LLM"),
        model_name=os.getenv("BACKBONE_LLM"),
        tmp=0.4,
        verbose=True,
    ) -> None:
    ...
    def __call__(
        self,
        user_query: str,
        literature_collection: LiteratureCollection = None,
        **kwargs,
    ):
        ...
        try:
            ...
            if (
                literature_collection is None
                or literature_collection.get_paper_count() == 0
            ):
                if self.verbose:
                    print(
                        "[OpenScholarReasoning] No literature found — reporting negative finding"
                    )
                return (
            ...
            if self.verbose:
                print(
                    f"[OpenScholarReasoning] Using {len(papers)} papers from {literature_collection.get_id()}"
                )
            ...
            for p in papers:
                if (
                    ...
                    or len(p["text"].strip()) == 0
                ):
                    if self.verbose:
                        print(
                            f"[OpenScholarReasoning] Skipping paper with missing/empty text: {p.get('title', 'Unknown')[:80]}",
                            flush=True,
                        )
                    continue
            ...
            if not sanitized_papers:
                if self.verbose:
                    print(
                        "[OpenScholarReasoning] No papers with valid text after sanitization",
                        flush=True,
                    )
                return (
            ...
            if self.verbose and len(sanitized_papers) != len(papers):
                print(
                    f"[OpenScholarReasoning] Sanitized: {len(papers)} papers → {len(sanitized_papers)} with valid text",
                    flush=True,
                )
            ...
            if not reasoning_output or reasoning_output == "No output generated":
                if self.verbose:
                    print(
                        f"[OpenScholarReasoning] OpenScholar failed to generate output"
                    )
            ...
            if self.verbose:
                print(
                    f"[OpenScholarReasoning] Created {reasoning_package.get_id()} successfully"
                )
    ...
    def __call__(self, reasoning_result: ReasoningPackage):
        ...
        if isinstance(reasoning_result, ReasoningPackage):
            if hasattr(reasoning_result, "reasoning") and reasoning_result.reasoning:
                ...
                )  # Return the dict containing reasoning results
            else:
    ...
    def __action_parser__(
        ...
        if "reasoning_result" in args:
            if args["reasoning_result"] is not None:
                for _, p_obs in reversed(action_chain):
                    if (
                        type(p_obs) == ReasoningPackage
                        and p_obs.get_id() == args["reasoning_result"]
                    ):
                        args["reasoning_result"] = p_obs
                        break
                    # Also check for direct OpenScholar responses (strings)
                    elif (
                        isinstance(p_obs, str)
                        and str(p_obs) == args["reasoning_result"]
                    ):
                        args["reasoning_result"] = p_obs
                        break
        ...
        if "literature_collection" in args:
            if args["literature_collection"] is not None:
                for _, p_obs in reversed(action_chain):
                    if (
                        type(p_obs) == LiteratureCollection
                        and p_obs.get_id() == args["literature_collection"]
                    ):
