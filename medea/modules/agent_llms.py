...
from ..tool_space.gpt_utils import (
    chat_completion,
    get_langchain_llm as _get_langchain_llm,
)
from ..tool_space.env_utils import (
    get_backbone_llm,
    get_env_with_error,
    get_llm_provider,
)
    ...
    def _extract_user_message(self, input_data: Union[str, Dict[str, Any]]) -> str:
        ...
        if isinstance(input_data, dict):
            ...
            if self.system_prompt and self.input_variables:
                missing_vars = [
                    var for var in self.input_variables if var not in input_data
                ]
                if missing_vars:
                    raise ValueError(
...
class LLMProviderRegistry:
    ...
    def set_provider(cls, provider: str) -> None:
        ...
        if provider not in VALID_LLM_PROVIDERS:
            print(
                f"[LLMProviderRegistry] ⚠️  Unknown provider '{provider}'. Valid: {', '.join(VALID_LLM_PROVIDERS)}"
            )
            return
        ...
        if arg_name in arguments and arguments[arg_name] is not None:
            ...
            if not re.match(pattern, value):
                ...
                if value.startswith("<Proposal:") or value.startswith("<CodeSnippet:"):
                    print(
                        f"[parse_action] ERROR: '{arg_name}' has extra text: '{value}'",
                        flush=True,
                    )
                    print(
                        f"[parse_action] Should be EXACTLY '<Proposal:xxxx>' or '<CodeSnippet:xxxx>' with NO additional text",
                        flush=True,
                    )
                return False
...
if __name__ == "__main__":
    ...
    llm = AgentLLM(llm_config=config, llm_name="gpt-4o", verbose=True)
