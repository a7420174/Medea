...
class ProposalToolSelector:
    ...
    def __call__(self, user_query: str, max_attempts: int = 3) -> List[Dict]:
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if attempt == max_attempts - 1:
                    print(
                        "Failed to select tools, using all available tools as fallback",
                        flush=True,
                    )
        ...
        print(
            f"Selected Tools for Proposal: {[tool.get('name') for tool in tool_info_json]}",
            flush=True,
        )
...
class ResearchPlanDraft(BaseAction):
    ...
    def _select_tools(self, user_query: str) -> List[Dict]:
        ...
        try:
            ...
            print(
                f"[ResearchPlanDraft] Selected {len(selected_tools)} tools out of {len(AVALIBLE_TOOL)} available",
                flush=True,
            )
            return selected_tools
        except Exception as e:
            print(
                f"[ResearchPlanDraft] Tool selection failed, using all tools: {e}",
                flush=True,
            )
...
class ContextVerification(BaseAction):
        ...
        try:
            ...
            checker_functions = {
                ...
                for name, obj in inspect.getmembers(
                    id_checkers_module, inspect.isfunction
                )
                if name.endswith("_checker")
            }

            print(
                f"Loaded {len(checker_functions)} checker functions: {list(checker_functions.keys())}",
                flush=True,
            )
    ...
    def _convert_null_to_none(self, obj):
        ...
        if isinstance(obj, dict):
            return {
                key: self._convert_null_to_none(value) for key, value in obj.items()
            }
        ...
        try:
            ...
            for pair in pairs:
                ...
                params_key = json.dumps(
                    pair.get("input_params", {}), sort_keys=True, default=str
                )
            ...
            if len(pairs) != len(unique_pairs):
                print(
                    f"[ContextVerification] Deduplicated {len(pairs)} pairs → {len(unique_pairs)} unique checks",
                    flush=True,
                )
                for (checker, _), tools in seen.items():
                    if len(tools) > 1:
                        print(
                            f"  {checker}: merged from {len(tools)} tools ({', '.join(tools)})",
                            flush=True,
                        )

            print(
                f"Context check output: {json.dumps(unique_pairs, indent=2)}",
                flush=True,
            )
            return unique_pairs
    ...
    def _prepare_parameters(
        self, checker_name: str, input_params: Dict, tool_name: str = None
    ) -> Dict:
        ...
        for param_name, param_def in param_config.items():
            if param_name in input_params:
                ...
                inferred_model = self._infer_model_from_tool(
                    tool_name, param_def.get("allowed_values", [])
                )
                if inferred_model:
        ...
        for param_name, param_def in param_config.items():
            ...
            if param_def.get("min_length") and isinstance(value, list):
                if len(value) < param_def["min_length"]:
                    errors.append(
                        f"{param_name} requires at least {param_def['min_length']} items"
                    )

            if param_def.get("allowed_values"):
                if isinstance(value, str) and value.lower() not in [
                    v.lower() for v in param_def["allowed_values"]
                ]:
                    errors.append(
                        f"{param_name} must be one of {param_def['allowed_values']}"
                    )
    ...
    def _validate_with_retries(
        self, proposal_text: str, attempts: int
    ) -> Tuple[List[str], bool]:
        ...
        for attempt in range(attempts):
            try:
                ...
                print(
                    f"[ContextVerification] Attempt {attempt + 1} failed: {e}",
                    flush=True,
                )
                if attempt == attempts - 1:
                    feedbacks = [
                        f"Context validation failed after {attempts} attempts: {str(e)}"
                    ]
                    all_valid = False

        return feedbacks, all_valid

    def _process_validation_pairs(
        self, pairs_to_check: List[Dict]
    ) -> Tuple[List[str], bool]:
        ...
        for pair in pairs_to_check:
            ...
            prepared_params = self._prepare_parameters(
                checker_name, input_params, tool_name
            )
    ...
    def _is_valid_checker_association(self, checker_name: str, tool_name: str) -> bool:
        ...
        if tool_name not in self.checker_to_tools.get(checker_name, []):
            print(
                f"Tool {tool_name} not associated with checker {checker_name}",
                flush=True,
            )
            return False
...
class IntegrityVerification(BaseAction):
    ...
    def __init__(
        self, llm_provider: str = None, tmp: float = 0.4, max_iter: int = 1
    ) -> None:
        # Get LLM provider with helpful error message if not provided
        if llm_provider is None:
            llm_provider = get_backbone_llm("gpt-4o")
    ...
    def _build_evaluation_prompt(
        self, proposal_draft: Proposal, previous_feedback: str, current_feedback: str
    ) -> str:
    ...
    def _evaluate_with_retries(
        self, prompt: str, proposal_draft: Proposal, max_retries: int = 3
    ) -> Proposal:
        """Evaluate proposal with retry logic."""
        for retry_count in range(max_retries):
            try:
                print(
                    f"[IntegrityVerification] Attempt {retry_count + 1}/{max_retries}",
                    flush=True,
                )
                feedback = self.llm_evaluater.run(prompt)
                print(
                    f"[IntegrityVerification] Raw response: {feedback[:200]}...",
                    flush=True,
                )
                ...
                print(
                    f"[IntegrityVerification] Error on attempt {retry_count + 1}: {e}",
                    flush=True,
                )

        print(
            f"[IntegrityVerification] Max retries reached, auto-approving proposal",
            flush=True,
        )
        proposal_draft.update_status("Approved")
        return proposal_draft
    ...
    def _process_feedback(self, feedback: str, proposal_draft: Proposal):
        ...
        if not match:
            print(
                f"[IntegrityVerification] Response doesn't match expected pattern",
                flush=True,
            )
        ...
        if status not in ["Failed", "Approved"]:
            print(
                f"[IntegrityVerification] Invalid status '{status}', expected 'Failed' or 'Approved'",
                flush=True,
            )
            return None
...
class ProposalFinishAction(BaseAction):
    ...
    def __init__(self) -> None:
        ...
        params_doc = {"proposal_draft": "the Proposal Object (e.g., <Proposal:xxxx>)"}
        super().__init__(
            action_name=action_name,
            action_desc=action_desc,
            params_doc=params_doc,
