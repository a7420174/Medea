...
try:
    ...
    print(
        "Warning: psutil not available. Process killing might be less effective.",
        flush=True,
    )
...
from medea.modules.experiment_analysis import (
    Analysis,
    CodeDebug,
    AnalysisExecution,
    CodeGenerator,
...
from medea.modules.literature_reasoning import (
    LiteratureSearch,
    OpenScholarReasoning,
    PaperJudge,
    LiteratureReasoning,
)
from medea.modules.research_planning import (
    ContextVerification,
    ResearchPlanDraft,
...
from medea.tool_space.env_utils import (
    get_env_with_error,
    get_seed,
    get_backbone_llm,
    get_llm_provider,
...
def check_model_cache(model_name: str, cache_dir: str = None) -> bool:
    ...
    model_cache_path = (
        Path(cache_dir) / "hub" / f"models--{model_name.replace('/', '--')}"
    )
    ...
    if is_cached:
        ...
        print(
            f"[MODEL CACHE] x {model_name} NOT in cache - will download on first use",
            flush=True,
        )
        print(
            f"[MODEL CACHE] Download progress bars are enabled (see logs below)",
            flush=True,
        )
...
LLM_JUDGE_MODEL = get_env_with_error(
    "EVALUATION_JUDGE_LLM", default=None, required=False
...
def medea_unittest(df, user_template=None, agent_template=None):
    ...
    research_plan_llm = AgentLLM(
        llm_config=research_plan_llm_config, llm_name=backbone_llm, verbose=True
    )
    ...
    analysis_llm = AgentLLM(
        llm_config=analysis_llm_config, llm_name=backbone_llm, verbose=True
    )
    ...
    literature_reason_llm = AgentLLM(
        llm_config=literature_reason_llm_config, llm_name=backbone_llm, verbose=True
    )
    ...
    research_plan_actions = [
        ...
        IntegrityVerification(
            tmp=research_plan_act_tmp,
            llm_provider=backbone_llm,
    ...
    analysis_actions = [
        ...
        AnalysisQualityChecker(
            tmp=analysis_act_tmp,
            llm_provider=backbone_llm,
            max_iter=CODE_QUALITY_MAX_ITER,
        ),
    ]
    ...
    literature_reason_actions = [
        ...
        OpenScholarReasoning(
            tmp=DEFAULT_TEMPERATURE, llm_provider=backbone_llm, verbose=True
        ),
    ]
    ...
    research_planning_module = ResearchPlanning(
        llm=research_plan_llm, actions=research_plan_actions
    )
    analysis_module = Analysis(llm=analysis_llm, actions=analysis_actions)
    literature_module = LiteratureReasoning(
        llm=literature_reason_llm, actions=literature_reason_actions
    )
    ...
    for candidate_genes, X, user_instruction, y, *attribute in inputs:
        try:
            ...
            experiment_instruction = (
                X[len(user_instruction) :].strip()
                if X.startswith(user_instruction)
                else None
            )
            ...
            acc_dict, cg_count, reason_count, hypo_count, gpt_count = (
                evaluate_prediction(
                    TASK,
                    y,
                    acc_dict,
            ...
            if "Incapsula Resource Error" in str(e):
                print(
                    "[medea_unittest] Incapsula Resource Error. Agent terminated.",
                    flush=True,
                )
                break
            continue
...
def llm_unitest(
    ...
    if mod == "multi_round_discussion":
        assert agent_output_df is not None, (
            "Agent output dataframe required for panel discussion mode"
        )
    ...
    for candidate_genes, _, user_instruction, y, *attribute in inputs:
        ...
        if mod == "llm":
            ...
            research_plan_text = agent_output[
                "proposal"
            ]  # Column name stays 'proposal' for compatibility
            ...
            literature_response_dict = {"user_query": reasoning_output}
            ...
            response = {"llm": llm_hypothesis_response, "final": hypothesis_response}
        ...
        log_df = log_saver(
            log_df, mod, TASK, candidate_genes, y, log_pack, TASK_LOG_FILE, *attribute
        )
        ...
        is_correct = (
            y.lower() in prediction_output.lower() if prediction_output else False
        )
...
def get_evaluation_data_path(task: str, query_mode: bool = True) -> str:
    ...
    task_config = {
        "targetID": {"query": TARGET_DB_QUERY_PATH, "source": TARGET_DB_PATH},
        "sl": {"query": SL_DB_QUERY_PATH, "source": SL_DB_PATH},
        "immune_response": {
...
def load_evaluation_data(
    task: str, query_mode: bool = True, checkpoint: str = None
) -> pd.DataFrame:
    """
    ...
    print(
        f"  Query mode: {query_mode} → Folder: {'evaluation_samples' if query_mode else 'source'}",
        flush=True,
    )
...
if __name__ == "__main__":
    ...
    paraphraser_llm_display = get_env_with_error(
        "PARAPHRASER_LLM", default=backbone_llm_display
    )
    ...
    df = load_evaluation_data(task=TASK, query_mode=QUERY_MODE, checkpoint=CHECKPOINT)
    ...
    if "medea" in SETTING:
        ...
        agent_output_df = pd.read_csv(
            os.path.join(EVALUATION_FOLDER, TASK, "source", AGENT_OUTPUT_PATH)
        )
        llm_unitest(
            df,
